...
function update_tovar($data)
{
    ...
    $tovarid  = intval($data->get_param('product'));

    $user = wp_get_current_user();
    $user_role = get_user_meta($user->ID);

    if ($user_role['role'] !== 'Admin' && $user_role['role'] !== 'Main admin') {
        ...
        if (!empty($category)) {
            $term = term_exists($category, 'custom_category'); // Замените 'Category_tovar' на вашу таксономию
            if ($term) {
